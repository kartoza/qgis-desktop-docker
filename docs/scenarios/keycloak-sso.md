...
flowchart LR
    ...
    Proxy <-->|"discovery + code exchange"| KC["Keycloak realm<br/>QGIS_DESKTOP_OIDC_ISSUER_URL"]
...
| Only entitled staff may open the desktop | `QGIS_DESKTOP_OIDC_ALLOWED_ROLES` / `_GROUPS` |
| The client secret is not visible to `docker inspect` | `QGIS_DESKTOP_OIDC_CLIENT_SECRET_FILE` + a `0400` mount |
...
| `docker exec <c> ls -l /run/qgis-desktop/oidc/secrets.cfg` | `-r-------- 1 user user` |
...
| Per-user Linux session | No | Yes | With `QGIS_DESKTOP_OIDC_INNER_MODE=greeter` |
| Works offline | Yes | Yes | No — needs the IdP |
| Clean re-prompt on failure | No | Yes | Yes |

`oidc` and `greeter` compose: set `QGIS_DESKTOP_OIDC_INNER_MODE=greeter` to require SSO
