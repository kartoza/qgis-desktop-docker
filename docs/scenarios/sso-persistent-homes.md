...
without touching the container. Open `qgis-homes/alice-3d9f21c8/deploy/` in the
MinIO console and upload it: within one interval it is on their desktop, owned
by them, and the prefix empties itself. `deploy/` is created at startup, so it is
already there to upload into.

For a baseline that arrives at every start and never overwrites the user's own
copy, create `baseline/` alongside it by uploading into that path. See
[Delivering data through the bucket](team-data-drop.md) for what each one is for
